...
def login_user(username, password, dob):
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute(
        "SELECT * FROM user WHERE username=%s AND password=%s AND dob=%s",
...
def signup_user(username, password, dob):
    ...
    try:
        cursor.execute(
            "INSERT INTO user (username, password, dob) VALUES (%s, %s, %s)",
            (username, hash_password(password), dob)
        )
        conn.commit()
        return True
...
def login_page():
    ...
    if st.button("Login"):
        if dob > datetime.date.today():
            ...
            user = login_user(username, password, dob)
            if user:
                st.session_state['authenticated'] = True
                st.session_state['username'] = username
            else:
                st.error("Incorrect username, password, or date of birth")

def signup_page():
    st.title("Signup Page")
    new_username = st.text_input("New Username")
    new_dob = st.date_input("Date of Birth", max_value=datetime.date.today())
    new_password = st.text_input("New Password", type="password")

    if st.button("Sign Up"):
        if new_dob > datetime.date.today():
            st.error("Date of Birth cannot be a future date.")
        else:
            if signup_user(new_username, new_password, new_dob):
